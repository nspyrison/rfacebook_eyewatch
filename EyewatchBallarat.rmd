...
The graphs below were created from the data of 952 posts of the Eyewatch Ballarat facebook page. The date was pulled on 16 May 2018.
...
tmp <- bal_posts_raw
  ...
  tmp$created_yearQuarter <- 
    paste0(as.integer(tmp$created_year), " Q", tmp$created_quarter)
  tmp$created_yearMonth <- 
    paste0(tmp$created_year, " ", tmp$created_month)
  tmp$created_yearMonth <- 
    zoo::as.yearmon(tmp$created_yearMonth, "%Y %b")
  tmp$created_yearWeek <- 
    paste0(tmp$created_year, " wk", tmp$created_week)
  tmp$created_weekday <- 
...
```
There is high variability in number of posts per month. On average 28 posts are made per month exluding the first 4 months.
...
sub <- gather(sub, key = created_yearMonth*type, value = likes_count,
                         comments_count, shares_count)

#TODO: FIX AGGREGATE OR MELT HERE
with(sub, aggregate(sub, list(created_yearMonth = created_yearMonth, type = type), sum)
...
## Link posts by weekday

```{r, echo=F}
sub <- dplyr::filter(bal_posts_clean, type == "link")
(p2 <- 
  ggplot(data = sub, 
         aes(x = created_weekday, fill = link_has_vicpol) ) + 
  geom_histogram(stat="count") + 
  ylab(label = "linkposts_count") + scale_fill_brewer(palette = "Dark2")
)  
```
...
sub <- 
  bal_posts_clean[
    c("created_date", "created_weekday", 
      "likes_count","comments_count", "shares_count", "type", "link_has_vicpol")
    ]
